...
apiRouter.post('/auth/register', async (req, res) => {
  ...
  res.status(201).json({ id: user.id, playerId: user.playerId, username: user.username });
});

apiRouter.post('/auth/login', async (req, res) => {
  const dto = loginSchema.parse(req.body);
  const user = await repositories.users.findByUsername(dto.username);
  if (!user || !(await bcrypt.compare(dto.password, user.passwordHash))) return res.status(401).json({ error: 'INVALID_CREDENTIALS' });
  user.isOnline = true; user.lastSeenAt = new Date().toISOString();
  await repositories.users.save(user);
  req.session.userId = user.id;
  res.json({ id: user.id, playerId: user.playerId, username: user.username });
});

apiRouter.post('/auth/logout', requireAuth, async (req, res) => {
  const user = await repositories.users.findById(req.session.userId!);
  if (user) { user.isOnline = false; await repositories.users.save(user); }
...
apiRouter.post('/decks/import', requireAuth, async (req, res) => res.status(201).json(await repositories.decks.importDeck(req.body)));
...
apiRouter.post('/matchmaking/join', requireAuth, async (req, res) => {
  ...
  if (picked.length >= 3) {
    const players = (await Promise.all(picked.map((p) => repositories.users.findById(p.userId)))).filter(Boolean).map((u) => ({
      userId: u!.id, playerId: u!.playerId, username: u!.username, score: 0, hand: [], ready: true, connected: true
    }));
    const match = await gameEngine.createMatch('QUEUE', players, {
      cardSelectSeconds: 30, judgeSeconds: 20, scoreLimit: 5, handSize: 7, language: 'ru',
      deckIds: ['base-ru', 'absurd-ru'], private: false, allowJoinByPlayerId: true, dedupeByText: false, enableGuestJudgeBot: true
    });
...
apiRouter.post('/matchmaking/leave', requireAuth, (req, res) => { matchmaking.leave(req.session.userId!); res.json({ ok: true }); });
...
apiRouter.get('/lobby/me', requireAuth, async (req, res) => {
  const user = await repositories.users.findById(req.session.userId!);
  if (!user) return res.status(401).json({ error: 'UNAUTHORIZED' });
  res.json(lobbyService.getOrCreate({ userId: user.id, playerId: user.playerId, username: user.username, score: 0, hand: [], ready: false, connected: true }));
});

...
export const registerSchema = z.object({ username: z.string().min(3), password: z.string().min(6), language: z.enum(['ru', 'en']).default('ru') });
export const loginSchema = z.object({ username: z.string().min(3), password: z.string().min(6) });
export const messageSchema = z.object({ text: z.string().min(1).max(240) });
export const joinByPlayerIdSchema = z.object({ playerId: z.string().min(2) });

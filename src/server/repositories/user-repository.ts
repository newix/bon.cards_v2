...
export class UserRepository {
  ...
  async create(username: string, passwordHash: string, language: 'ru' | 'en'): Promise<User> {
    ...
    const playerId = `P${Math.random().toString(36).slice(2, 8).toUpperCase()}`;
    if (users.some((u) => u.playerId === playerId)) throw new Error('PLAYER_ID_COLLISION');
    const now = new Date().toISOString();
    const user: User = {
      id: randomUUID(), playerId, username, passwordHash,
      createdAt: now, lastSeenAt: now, isOnline: false,
